...
class AzureApi():
    ...
    def create_subnet(self, vnet_name='AzureVneteastasia', sub_name='cdntip'):
        ...
        # print('1111', self.report_get(url))
        if not self.report_get(url): return False
    ...
    def reset_nic(self, nic_name='cdntip'):
        try:
            ...
            for num in range(20):
                self.report_put_json(url, data)
                if self.result.get('error', 'sss') in ['sss']:
                    print(self.result)
                    break
...
def create_vm(client_id, client_secret, tenant, subscriptionId):
    ...
    if not azApi.create_group(group_name):
        print('获取 创建资源组失败 失败')
        return False

    # 2, 创建 虚拟网络 和 子网
    vnet_status = azApi.create_virtual_networks(group_name=group_name, location=location)
    if not vnet_status:
        print('创建 虚拟网络 失败')
        return False

    # 创建子网
    if not azApi.create_subnet(vnet_name=azApi.vnet_name, group_name=group_name, sub_name=location):
        print('创建 虚拟网络-子网 失败')
        return False

    # 3， 创建公共IP
    if not azApi.create_public_ip(group_name=group_name, location=location, public_ip_name=vm_name):
        print('创建 公共IP 失败')
        return False

    # 4, 创建网络接口
    if not azApi.create_nic(group_name=group_name, location=location, nic_name=vm_name, public_ip_id=azApi.public_ip_id, subnet_id=azApi.sub_id):
        print('创建 公共IP 失败')
        return False

    # 5, 创建虚拟机
    if not azApi.create_vm(location=location, group_name=group_name, vm_name=vm_name, nic_id=azApi.nic_id):
        print('创建 虚拟机 失败')
        return False
